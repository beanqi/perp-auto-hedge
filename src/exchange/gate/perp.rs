...
impl GatePerp{
    ...
    pub fn run_market_stream(&self,symbols:&[String],tx:Sender<MarketEvent>){if symbols.is_empty(){return;}if self.ensure_contracts().is_err(){return;}loop{if let Err(e)=self.market_connection(symbols,&tx){logging::info(&format!("gate market websocket reconnecting: {e}"));}thread::sleep(RECONNECT);}}
    ...
    fn market_connection(&self,symbols:&[String],tx:&Sender<MarketEvent>)->ExchangeResult<()> {let mut ws=connect_ws()?;for ch in ["futures.book_ticker","futures.tickers"]{ws.send(Message::Text(json!({"time":now_secs(),"channel":ch,"event":"subscribe","payload":symbols}).to_string().into()))?;}
        loop{match ws.read()?{Message::Text(raw)=>{let v:Value=serde_json::from_str(raw.as_ref())?;if text(&v,"event")!=Some("update"){continue;}match text(&v,"channel"){Some("futures.book_ticker")=>if let Some(e)=self.parse_book(&v["result"]){tx.send(e).map_err(|_|err("market receiver closed"))?;},Some("futures.tickers")=>if let Some(rows)=v["result"].as_array(){for row in rows{if let Some(e)=parse_funding(row){tx.send(e).map_err(|_|err("market receiver closed"))?;}}},_=>{}}},Message::Ping(x)=>ws.send(Message::Pong(x))?,Message::Close(_)=>return Ok(()),_=>{}}}}
    ...
    fn parse_book(&self,v:&Value)->Option<MarketEvent>{let s=text(v,"s")?;let meta=self.contracts.read().ok()?.get(s).copied()?;Some(MarketEvent::OrderBook(OrderBook{market_id:market(s),bids:vec![PriceLevel{price:number(v,"b")?,quantity:number(v,"B")?*meta.multiplier}],asks:vec![PriceLevel{price:number(v,"a")?,quantity:number(v,"A")?*meta.multiplier}],timestamp_ms:integer(v,"t").unwrap_or_else(now_ms),sequence:integer(v,"u")}))}
...
fn connect_ws()->ExchangeResult<Ws>{let mut r=WS_URL.into_client_request()?;r.headers_mut().insert("X-Gate-Size-Decimal",HeaderValue::from_static("1"));Ok(connect(r)?.0)}
...
#[cfg(test)] mod tests{use super::*;#[test]fn contract_quantity_is_converted(){let a=GatePerp::new(None);a.contracts.write().unwrap().insert("BTC_USDT".into(),ContractMeta{multiplier:0.0001,min_native:1.0,decimal:true});let MarketEvent::OrderBook(b)=a.parse_book(&json!({"s":"BTC_USDT","b":"100","B":"10","a":"101","A":"20","t":1})).unwrap()else{panic!()};assert_eq!(b.bids[0].quantity,0.001);}}
