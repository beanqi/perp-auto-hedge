...
impl GatePerp{
    ...
    pub fn start_market_streams(&self,symbols:&[String],tx:Sender<SystemEvent>)->ExchangeResult<()> {if symbols.is_empty(){return Ok(());}self.ensure_contracts()?;for(index,chunk)in symbols.chunks(MARKET_SYMBOLS_PER_CONNECTION).enumerate(){let exchange=self.clone();let symbols=chunk.to_vec();let tx=tx.clone();thread::Builder::new().name(format!("gate-book-{index}")).spawn(move||exchange.run_book_stream(&symbols,tx))?;}let exchange=self.clone();let symbols=symbols.to_vec();thread::Builder::new().name("gate-funding".into()).spawn(move||exchange.run_funding_stream(&symbols,tx))?;Ok(())}
    pub fn run_book_stream(&self,symbols:&[String],tx:Sender<SystemEvent>){if symbols.is_empty(){return;}loop{if let Err(e)=self.market_connection(symbols,&tx){logging::info(&format!("gate SBE market websocket reconnecting: {e}"));}thread::sleep(RECONNECT);}}
    pub fn run_private_stream(&self,tx:Sender<SystemEvent>){if self.ensure_contracts().is_err(){return;}loop{if let Err(e)=self.private_connection(&tx){logging::info(&format!("gate private websocket reconnecting: {e}"));}thread::sleep(RECONNECT);}}
    ...
    fn market_connection(&self,symbols:&[String],tx:&Sender<SystemEvent>)->ExchangeResult<()> {let mut ws=connect_sbe_ws()?;ws.send(Message::Text(json!({"time":now_secs(),"channel":"futures.book_ticker","event":"subscribe","payload":symbols}).to_string().into()))?;
        loop{match ws.read()?{Message::Binary(raw)=>if let Some(e)=self.parse_sbe_book(raw.as_ref())?{tx.send(SystemEvent::Market(e)).map_err(|_|err("market receiver closed"))?;},Message::Text(raw)=>{let v:Value=serde_json::from_str(raw.as_ref())?;if let Some(e)=v.get("error").filter(|x|!x.is_null()){return Err(err(format!("gate SBE subscription failed: {e}")));}},Message::Ping(x)=>ws.send(Message::Pong(x))?,Message::Close(_)=>return Ok(()),_=>{}}}}

    fn run_funding_stream(&self,symbols:&[String],tx:Sender<SystemEvent>){loop{if let Err(e)=self.funding_connection(symbols,&tx){logging::info(&format!("gate funding websocket reconnecting: {e}"));}thread::sleep(RECONNECT);}}
    fn funding_connection(&self,symbols:&[String],tx:&Sender<SystemEvent>)->ExchangeResult<()> {let mut ws=connect_ws()?;ws.send(Message::Text(json!({"time":now_secs(),"channel":"futures.tickers","event":"subscribe","payload":symbols}).to_string().into()))?;
        loop{match ws.read()?{Message::Text(raw)=>{let v:Value=serde_json::from_str(raw.as_ref())?;if text(&v,"event")!=Some("update")||text(&v,"channel")!=Some("futures.tickers"){continue;}if let Some(rows)=v["result"].as_array(){for row in rows{if let Some(e)=parse_funding(row){tx.send(SystemEvent::Market(e)).map_err(|_|err("market receiver closed"))?;}}}},Message::Ping(x)=>ws.send(Message::Pong(x))?,Message::Close(_)=>return Ok(()),_=>{}}}}

    fn private_connection(&self,tx:&Sender<SystemEvent>)->ExchangeResult<()> {let account=self.signed_get("/futures/usdt/accounts","")?;let uid=account.get("user").and_then(scalar).ok_or_else(||err("gate user id missing"))?;let mut ws=connect_ws()?;self.subscribe_private(&mut ws,"futures.balances",vec![uid.clone()])?;for ch in ["futures.positions","futures.orders","futures.usertrades"]{self.subscribe_private(&mut ws,ch,vec![uid.clone(),"!all".into()])?;}
        loop{match ws.read()?{Message::Text(raw)=>{let v:Value=serde_json::from_str(raw.as_ref())?;if text(&v,"event")!=Some("update"){continue;}match text(&v,"channel"){
            Some("futures.balances")=>for b in self.fetch_balances()?{tx.send(SystemEvent::Account(AccountEvent::Balance(b))).map_err(|_|err("account receiver closed"))?;},
            Some("futures.positions")=>for p in self.fetch_positions()?{tx.send(SystemEvent::Account(AccountEvent::Position(p))).map_err(|_|err("account receiver closed"))?;},
            Some("futures.orders")=>{let m=self.contracts.read().map_err(|_|err("gate contract lock poisoned"))?;if let Some(rows)=v["result"].as_array(){for row in rows{if let Some(o)=parse_order(row,&m){tx.send(SystemEvent::Account(AccountEvent::Order(o))).map_err(|_|err("account receiver closed"))?;}}}},
            Some("futures.usertrades")=>{let m=self.contracts.read().map_err(|_|err("gate contract lock poisoned"))?;if let Some(rows)=v["result"].as_array(){for row in rows{if let Some(f)=parse_fill(row,&m){tx.send(SystemEvent::Account(AccountEvent::Fill(f))).map_err(|_|err("account receiver closed"))?;}}}},_=>{}}},Message::Ping(x)=>ws.send(Message::Pong(x))?,Message::Close(_)=>return Ok(()),_=>{}}}}
